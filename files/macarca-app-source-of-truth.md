...
## 1a. Full MVP Roadmap (permanent reference — does not change per session)

This is the complete approved plan, kept here so any future session sees the whole picture, not just the current sprint.

| Sprint | Goal | Output |
|---|---|---|
| **Sprint 0** | Project setup | Empty React app running locally, Git/GitHub initialized |
| **Sprint 1** | Vocabulary core loop | Home → Category List → Word List, with 1 sample category (e.g. Greetings, ~10 words) |
| **Sprint 2** | Sentences core loop | Same pattern applied to Daily Sentences |
| **Sprint 3** | Review Mode | Flip-card review pulling from vocabulary/sentences data |
| **Sprint 4** | Styling & mobile pass | Clean, consistent, mobile-friendly UI across all screens |
| **Sprint 5+ (post-MVP, product owner's call)** | Progress tracking, search, more content, audio | Only after MVP is validated and explicitly approved |

**MVP feature scope (approved, for reference):**
- ✅ In MVP: Vocabulary browser (by category), Daily Sentences list, simple flashcard-style review
- 🟡 Possible v1.1 (not decided yet): progress tracking, search/filter
- 🔴 Explicitly excluded from MVP: audio pronunciation, user accounts/login, spaced repetition algorithm, gamification, social features, backend/database

**Approved tech stack (permanent, does not change per session unless explicitly revisited):**
React + Vite (JS, not TS) · Plain CSS · Local JSON content · No backend/database/auth · Simple state-based navigation (React Router deferred unless genuinely needed) · Public GitHub repo

---

## 1b. Sprint History (running log — append each session, never delete)

- **Sprint 0 (Session 1):** Project setup — Vite+React scaffold created, environment verified (Node v24.15.0/npm 11.12.1/Git 2.54.0, all pre-existing), Git initialized, GitHub repo `Apollo35/macarca-app` created (public) and pushed successfully. **✅ Complete**
...
*This document reflects verified project state only. Nothing above has been assumed or invented — all items marked complete were confirmed via terminal output or explicit user confirmation.*
